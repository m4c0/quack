...
namespace quack::donald {
  ...
  void atlas(jute::view res_name) { js::load_texture(res_name.begin(), res_name.size()); }
  ...
  void data(buffer_fn_t d) {
    ...
    if (qty > 0) js::bind_instances(qty, g_quads.begin(), g_quads.size());
